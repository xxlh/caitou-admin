...
import { OrderItemType } from './data';
...
export async function getGoods(id:number) {
  return request<{product:OrderItemType}>(`/admin/products/${id}`, {
    method: 'GET',
  });
}
...
export async function updateGoods(id:number, data?: { [key: string]: any }) {
  return request<OrderItemType>(`/admin/products/${id}`, {
    method: 'PATCH',
    data,
  });
}
...
export async function addGoods(data?: { [key: string]: any }) {
  return request<OrderItemType>('/admin/products', {
    method: 'POST',
    data,
  });
}
