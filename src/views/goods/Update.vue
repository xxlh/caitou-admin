      ...
      <a-form :form="form" @submit="handleSubmit" :selfUpdate="true">
        ...
        <div class="tabs-content">
          ...
          <div class="tab-pane" v-show="tabKey == 0">
            ...
            <template v-if="form.getFieldValue('type') === 'course'">
              ...
              <a-form-item label="课程媒体" :labelCol="labelCol" :wrapperCol="wrapperCol">
                <SelectAudio
                  ...
                  :multiple="false"
                  :defaultList="formData.goods.course_media ? [formData.goods.course_media] : []"
            ...
            <a-form-item v-if="form.getFieldValue('type') !== 'course'" label="配送所需" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-radio-group
            ...
            <a-form-item v-if="form.getFieldValue('type') !== 'course'" label="最晚配送" :labelCol="labelCol" :wrapperCol="wrapperCol" extra="超过最晚配送时间后，商品自动下架">
          ...
          <div class="tab-pane" v-show="tabKey == 1">
            ...
            <div v-show="form.getFieldValue('spec_type') == 20 && form.getFieldValue('type') !== 'course'">
              <MultiSpec ref="MultiSpec" :goodsId="goodsId"
                :defaultSpecList="formData.specs"
                :defaultSkuList="formData.skus"
              />
            </div>
            <!-- 单规格的表单内容 -->
          ...
          <div class="tab-pane" v-show="tabKey == 2">
            <a-form-item label="商品详情" :labelCol="labelCol" :wrapperCol="{span: 16}">
              <Ueditor :channel="channel" :channel_id="goodsId" collection="desc_images" v-decorator="['description', {rules: [{required: true, message: '商品详情不能为空'}]}]" />
            </a-form-item>
          </div>
...
export default {
  ...
  methods: {
    ...
    initData () {
      ...
      // 获取form所需的数据
      this.isLoading = true
      GoodsModel.getFormData(this.goodsId)
        .then(() => {
          // 商品表单数据
          const fieldsValue = GoodsModel.getFieldsValue()
          if (!isEmptyObject(this.form.getFieldsValue())) {
            this.form.setFieldsValue(fieldsValue)
          }
          this.isCourseFree = fieldsValue.type === 'course' && Number(fieldsValue.price) === 0
          // 设置课程媒体类型（根据已有数据或默认值）
          this.courseMediaType = fieldsValue.course_media_type || 'audio'
    ...
    onTargetTabError (errors) {
      ...
      // 只需要必填字段就可
      const tabsFieldsMap = [
        ['title', 'categorys', 'image_ids', 'course_media_type', 'course_media_id', 'course_trial_minutes'],
        ['price'],
        ['description'],
    ...
    */
    onFormSubmit (values) {
      ...
      Promise.all([
        ...
        .then(() => {
          GoodsApi.edit(this.goodsId, values)
            .then(result => {
              // 显示提示信息
              this.$message.success('商品已更新！', 1.5)
              // 跳转到列表页
              setTimeout(() => {
